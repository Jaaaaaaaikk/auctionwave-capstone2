            ...
            <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
                <div class="mx-auto max-w-5xl">
                    <div class="gap-4 sm:flex sm:items-center sm:justify-between">
                        <h2 class="text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl">My Transactions</h2>

                        <div class="mt-6 gap-4 space-y-4 sm:mt-0 sm:flex sm:items-center sm:justify-end sm:space-y-0">
                            <div>
                                <select id="order-type" v-model="selectedTransactionStatus"
                                    @change="resetTransactionStatus"
                                    ...
                                    <option v-for="offerStatus in transactionStatusMap" :key="offerStatus"
                        ...
                        <div class="divide-y divide-gray-300 dark:divide-gray-700">
                            ...
                            <div v-else v-for="auction in filteredAuctions" :key="auction.listing_id"
                                ...
                                <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                    ...
                                    <dd :class="getStatusClass(auction.transaction_status)"
                                        class="me-2 mt-1.5 inline-flex items-center rounded px-2.5 py-0.5 text-xs font-medium">
                                        <svg v-if="auction.transaction_status === 'Transaction Pending'"
                                        ...
                                        <svg v-if="auction.transaction_status === 'Transaction Completed'"
                                            class="me-1 h-3 w-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                            width="24" height="24" fill="none" viewBox="0 0 24 24">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                                stroke-width="2" d="M5 11.917 9.724 16.5 19 7.5" />
                                        </svg>
                                        <svg v-if="auction.transaction_status === 'Transaction Failed'"
                                            class="me-1 h-3 w-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                            width="24" height="24" fill="none" viewBox="0 0 24 24">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                    ...
                                    <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">{{
                                        formatDate(auction.transaction_time) }}
                                    </dd>
                                </dl>
                                ...
                                <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                    <div
                                        ...
                                        <button @click="handleAction(auction)"
                                            class="w-full inline-flex justify-center rounded-lg border border-gray-200 bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-4 text-sm font-medium hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700 lg:w-auto">
                                ...
                                <BidderRatingModal v-if="bidderRatingModal"
                                    @closeRatingModal="bidderRatingModal = false"
...
const noResultsMessage = computed(() => {
    if (selectedTransactionStatus.value) {
        return "No transactions found for the selected status.";
    } else {
        return "No transactions found for your selected status.";
    }
});
...
const handleAction = async (auction) => {
    ...
    const canRate = await checkIfAuctionRated(auction.listing_id, auction.auctioneer_id);

    if (canRate) {
        bidderRatingModal.value = true;
    } else {
...
const fetchTransactionAuctions = async () => {
    ...
    try {
        const { data } = await axios.get("/api/auctions/display-transaction-history", {
            params: { page: currentPage.value, numberOfAuctions: numberOfAuctions.value, selectedTransactionStatus: selectedTransactionStatus.value, }
        });
        console.log("Fetched Transaction data result:", data);
        auctions.value = data.transactions;
...
const filteredAuctions = computed(() => {
    ...
    return auctions.value.filter((auction) => {
        ...
        return matchesTransactionStatus;
    });
});
