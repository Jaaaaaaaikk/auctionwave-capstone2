                            ...
                            <div v-else v-for="auction in filteredAuctions" :key="auction.listing_id"
                                ...
                                <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                                    ...
                                    <dd :class="getStatusClass(auction.status)"
                                        ...
                                        <svg v-if="auction.status === 'CashBond Pending' || auction.status === 'Transaction Pending' || auction.status === 'Offer Pending' || auction.status === 'Active'"
                                        ...
                                        <svg v-if="auction.status === 'Offer Accepted' || auction.status === 'CashBond Top-up Completed'"
                                            class="me-1 h-3 w-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                            width="24" height="24" fill="none" viewBox="0 0 24 24">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                                stroke-width="2" d="M5 11.917 9.724 16.5 19 7.5" />
                                        </svg>
                                        <svg v-if="auction.status === 'Outbid' || auction.status === 'Offer Discarded' || auction.status === 'CashBond Response Failed'"
                                            class="me-1 h-3 w-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                            width="24" height="24" fill="none" viewBox="0 0 24 24">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
...
const bidStatusMap = ref([
    "Active",
    "Outbid",
    "CashBond Top-up Completed",
    "CashBond Pending",
    "Transaction Pending",
    "CashBond Response Failed"
]);
...
const getStatusClass = (status) => {
    switch (status) {
        ...
        case 'CashBond Pending':
        case 'Transaction Pending':
        case 'Offer Pending':
        case 'Active':
            return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300';
        case 'Offer Accepted':
        case 'CashBond Top-up Completed':
            return 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300';
        case 'CashBond Response Failed':
        case 'Outbid':
...
const isButtonDisabled = (status) => {
    const disabledStatuses = ['CashBond Response Failed', 'Offer Discarded'];
...
const handleAction = (auction) => {
    ...
    } else if (auction.status === 'CashBond Top-up Completed') {
