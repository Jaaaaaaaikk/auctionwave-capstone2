<template>
    <NuxtLayout name="biddernavbar">
        ...
        <div v-else-if="!isLoading" class="bg-custom-blue2 rounded-md overflow-hidden w-3/4 mx-auto mt-4 px-10 ml-72 ">
            <div class="font-semibold text-2xl rounded-md overflow-hidden w-full  mt-4">
                <h1>Leaderboards</h1>
            </div>

            <div class="bg-custom-blue2  rounded-md overflow-hidden w-full mx-auto mt-4 my-10 ">
                <div class="bg-gray-100 py-2 px-4">
                    <h2 class="text-xl font-semibold text-gray-800">Top Auctioneers</h2>
                </div>

                <table v-if="auctioneerData.length > 0" class="min-w-full bg-white">
                    <!-- Column headers -->
                    <thead>
                        <tr class="bg-gray-200">
                            <th class="w-1/12 py-2 px-6 text-left text-gray-700 text-lg font-medium">#</th>
                            <th class="w-3/12 py-2 px-6 text-left text-gray-700 text-lg font-medium">Name</th>
                            <th class="w-3/12 py-2 px-6 text-left text-gray-700 text-lg font-medium">Location</th>
                            <th class="w-3/12 py-2 px-6 text-left text-gray-700 text-lg font-medium">Successful Auctions
                            </th>
                            <th class="w-2/12 py-2 px-6 text-left text-gray-700 text-lg font-medium">Rating</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        <!-- Row 1 -->
                        <tr v-for="(auctioneer, index) in auctioneerData" :key="index">
                            <td class="py-4 px-6 text-gray-700 text-lg font-medium">{{ index + 1 }}</td>
                            <td class="py-4 px-6 flex items-center">
                                <img class="w-12 h-12 rounded-full object-cover mr-4"
                                    :src="auctioneer.profile_image || '/images/default-profile-image.png'"
                                    alt="User Image">
                                <span class="text-lg font-medium text-gray-800 hover:underline hover:cursor-pointer">{{
                                    auctioneer.name }}</span>
                            </td>
                            <td class="py-4 px-6 text-gray-600 text-base">{{ auctioneer.location }}</td>
                            <td class="py-4 px-6 text-gray-600 text-base">{{ auctioneer.successful_auctions }}</td>
                            <td class="py-4 px-6 flex items-center">
                                <svg class="w-4 h-4 text-yellow-300 mr-1" xmlns="http://www.w3.org/2000/svg"
                                    fill="currentColor" viewBox="0 0 22 20">
                                    <path
                                        d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z" />
                                </svg>
                                <span class="text-sm font-bold text-gray-900">{{ auctioneer.average_rating ?
                                    auctioneer.average_rating.toFixed(1) : 0.00 }}</span>
                                <span class="w-1 h-1 mx-1.5 bg-gray-500 rounded-full"></span>
                                <a href="#" class="text-sm font-medium text-gray-900 underline hover:no-underline">{{
                                    auctioneer.total_reviews }} reviews</a>
                            </td>
                        </tr>

                    </tbody>
                </table>
                <p v-else class="py-6 px-4 text-gray-600 text-center">No top auctioneers found.</p>
            </div>



            <div class="bg-white  rounded-md overflow-hidden w-full mx-auto mt-4 my-10 ">
                <div class="bg-gray-100 py-2 px-4">
                    <h2 class="text-xl font-semibold text-gray-800">Top Bidders</h2>
                </div>

                <table v-if="bidderData.length > 0" class="min-w-full bg-white">
                    <!-- Column headers -->
                    <thead>
                        <tr class="bg-gray-200">
                            <th class="w-1/12 py-2 px-6 text-left text-gray-700 text-lg font-medium">#</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <!-- Row 1 -->
                        <tr v-for="(bidder, index) in bidderData" :key="index">
                            <td class="py-4 px-6 text-gray-700 text-lg font-medium">{{ index + 1 }}</td>
                            <td class="py-4 px-6 flex items-center">
                                <img class="w-12 h-12 rounded-full object-cover mr-4"
                                    :src="bidder.profile_image || '/images/default-profile-image.png'" alt="User Image">
                                <span class="text-lg font-medium text-gray-800 hover:underline hover:cursor-pointer">{{
                                    bidder.name }}</span>
                            </td>
                            <td class="py-4 px-6 text-gray-600 text-base">{{ bidder.location }}</td>
                            <td class="py-4 px-6 text-gray-600 text-base">{{ bidder.successful_auctions }}</td>
                            <td class="py-4 px-6 flex items-center">
                                <svg class="w-4 h-4 text-yellow-300 mr-1" xmlns="http://www.w3.org/2000/svg"
                                    fill="currentColor" viewBox="0 0 22 20">
                                    <path
                                ...
                                <span class="text-sm font-bold text-gray-900">{{ bidder.average_rating ?
                                    bidder.average_rating.toFixed(1) : 0.0 }}</span>
                                <span class="w-1 h-1 mx-1.5 bg-gray-500 rounded-full"></span>
                                <span class="text-sm font-medium text-gray-900 underline hover:no-underline">{{
                                    bidder.total_reviews }} reviews</span>
                            </td>
                        </tr>

                    </tbody>
                </table>
                <p v-else class="py-6 px-4 text-gray-600 text-center">No top bidders found.</p>
            </div>
        </div>
        <!-- bidder -->
...
const fetchLeaderboards = async () => {
    ...
    try {
        const response = await axios.get("/api/ratings/get-bidder-leaderboards");

        console.log('response on leaderboards', response.data);
        bidderData.value = response.data.bidders || [];
        auctioneerData.value = response.data.auctioneers || [];
    } catch (error) {
