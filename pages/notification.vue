<template>
    ...
    <NuxtLayout :name="navbarLayout">
        ...
        <div class="bg-gray-100 mx-4 rounded-lg">
            ...
            <!-- Pagination Controls -->
            <div v-if="notificationsStore.pagination.totalPages > 1" class="flex justify-center space-x-2 py-4">
                <!-- Previous Page Button -->
                <button @click="changePage(notificationsStore.pagination.page - 1)"
                    :disabled="notificationsStore.pagination.page <= 1" class="btn">Previous</button>
                <!-- Page Number Buttons -->
                <span v-for="page in pageNumbers" :key="page">
                    <button @click="changePage(page)"
                        :class="{ 'bg-blue-500': page === notificationsStore.pagination.page }" class="btn">
                        {{ page }}
                    </button>
                </span>
                <!-- Next Page Button -->
                <button @click="changePage(notificationsStore.pagination.page + 1)"
                    :disabled="notificationsStore.pagination.page >= notificationsStore.pagination.totalPages"
...
const pageNumbers = computed(() => {
    const pages = [];
    for (let i = 1; i <= notificationsStore.pagination.totalPages; i++) {
        pages.push(i);
    }
    return pages;
});
...
const changePage = (page) => {
    if (page < 1 || page > notificationsStore.pagination.totalPages) return; // Prevent invalid pages
    notificationsStore.fetchNotifications(page);
};

const getClickHandler = (notification) => {
    const isSystemSender = notification.sender_full_name?.includes("AuctionWave System");
    if (isSystemSender) {
