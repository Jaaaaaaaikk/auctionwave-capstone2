    ...
    <section class="bg-white rounded-lg  flex flex-col items-center justify-center w-5/6 mx-auto">
      ...
      <div v-else class="w-full">
        ...
        <div v-if="isSearching && filteredAuctions.length === 0"
          ...
          <div v-if="selectedAuctionStatus">
            No auctions found for the selected auction status.
          </div>
          <div v-else-if="selectedPaymentStatus">
            No auctions found for the selected payment status.
          </div>
          <div v-else-if="selectedTransactionStatus">
            No auctions found for the selected transaction status.
          </div>
          <div v-else>
            No auctions found for your search criteria.
          </div>
        </div>
        <!-- Table Layout -->
        <div v-for="auction in filteredAuctions" :key="auction.listing_id" @click="view_auction(auction)"
          ...
          <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
            ...
            <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">
              {{ truncateDescription(auction.status, 100) }}
            </dd>
          </dl>
