...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        if (!token) {
            throw createError({
                ...
                data: {
                    ...
                    errorCode: 'UNAUTHORIZED_NOT_FOUND_TOKEN',
                }
            });
        }
        ...
        const { auctionUuid, orderID } = body;
        ...
        await pool.query(
            `INSERT INTO Payments (listing_id, user_id, amount,  payment_type, order_id) VALUES (?, ?, ?, ?, ?)`,
            [listingId, user_id, 150, 'Emailblast Fee', orderID]
        );
        ...
        throw createError({
            statusCode: error.statusCode || 500, statusMessage: error.message || 'Internal Server Error',
            data: {
                success: false,
                message: 'Internal Server Error',
                errorCode: 'ERROR_INTERNAL_SERVER_500',
            }
        });
    }
});
