<template>
  <div class="bg-custom-blue2 dark:bg-gray-900 min-h-screen">
    <!-- Header Section -->
    <div class="fixed top-0 left-0 right-0 flex items-center justify-between bg-white shadow-md mb-6 z-20  lg:py-0">
      <NuxtLink to="/bidder/bidderdashboard"
        class="hidden lg:flex justify-start space-x-3 rtl:space-x-reverse mr-8 lg:ml-7 flex-shrink-0">
        <img src="/public/images/logo-no-text123.jpg" class="h-16" alt="Logo" />
      </NuxtLink>
      <!-- collapsible sidebar in mobile devices -->
      <div class="">
            ...
            <ul class="space-y-2 font-medium">
              ...
              <li>
                <NuxtLink to="/bidder/bidding-activity">
                  <div
                    class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group mb-4">
                    <svg viewBox="0 0 1024 1024" class="w-6 h-6" version="1.1" xmlns="http://www.w3.org/2000/svg"
                      fill="#000000">
      ...
      <div class="flex items-center w-full ">
        <form v-if="isBidderDashboard"
          class="xl:ml-74.5 md:ml-24 lg:ml-52 sm:w-3/5 md:w-2/3 lg:w-1/2 xl:w-1/2 2xl:w-3/5 mx-4 w-full">
          ...
          <div class="relative">
            ...
            <input type="search" id="default-search" v-model="auctionStore.searchTerm"
              ...
              placeholder="Search auction name..." required @focus="filtersOpen = true" @blur="handleBlur" />

            <div v-if="filtersOpen"
              class="absolute z-10 w-full bg-white border border-gray-300 rounded-lg shadow-lg dark:bg-gray-700 dark:border-gray-600 sm:block  lg:hidden">
              <!-- Filters List -->
              ...
              <div @click="filtersOpen = false" class="flex justify-center py-2 bg-custom-bluegreen rounded-b-lg">
                <button class="p-2 text-gray-600 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-600 rounded-full">

                  <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-white dark:text-white" fill="none"
        ...
        <div class=" lg:flex space-x-4 items-center flex-grow justify-end">
          ...
          <div class="relative pr-8 flex-shrink-0 hidden lg:block">
...
const handleIncomingSystemNotification = (systemNotification) => {
  ...
  toast(`From: ${systemNotification.notification.sender_full_name}: ${systemNotification.notification.message}.`, {
    type: 'info',
    autoClose: 3000,
    position: 'top-right',
  });
...
const handleIncomingResponseFromOutbid = (outbidNotification) => {
  ...
  toast(`From ${outbidNotification.notification.sender_full_name}: ${outbidNotification.notification.message}.`, {
    type: 'info',
    autoClose: 3000,
    position: 'top-right',
  });
