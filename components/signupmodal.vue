  ...
  <transition name="modal-width">
    <div class="bg-white rounded-lg shadow relative dark:bg-gray-700 justify-center">
      <form class="space-y-6 px-6 lg:px-8 pb-4 sm:pb-6 xl:pb-8 pt-8">
        ...
        <span v-if="passwordError" class="text-red-500 text-xs ml-3">{{ passwordError }}</span>
        <!-- User Type (Combobox) -->
        <div class="w-full px-2 mb-4 sm:mb-0 relative">
...
watch([() => form.value.password, () => form.value.confirmPassword], ([newPassword, newConfirmPassword]) => {
  ...
  if (newPassword && !passwordRegex.test(newPassword) || newConfirmPassword && !passwordRegex.test(newConfirmPassword)) {
    passwordError.value = "Password must be at least 8 characters long, contain a number, and a special character (e.g., @, $, !).";
  } else if (newConfirmPassword !== newPassword) {
    passwordError.value = "Passwords do not match.";
  } else {
    passwordError.value = ""; // Clear the error if both conditions are met
  }
});
...
const validateForm = () => {
  ...
  if (form.value.password && !passwordRegex.test(form.value.password)) {
    toast.warning("Password must be at least 8 characters long, contain a number, and a special character (e.g., @, $, !).");
    return false;
  }

  if (form.value.confirmPassword && !passwordRegex.test(form.value.confirmPassword)) {
    toast.warning("Confirm Password must be at least 8 characters long, contain a number, and a special character (e.g., @, $, !).");
    return false;
  }
