        ...
        <section class="p-4">
          ...
          <div class="mt-6 flex justify-center">
            <div v-if="uploadedImage" class="border rounded-lg p-2">
              <img :src="uploadedImage" alt="User Uploaded Image" class="w-full h-32 object-cover rounded-md" />
            </div>
          </div>
        </section>
        ...
        <section class="p-4">
          ...
          <div v-if="previousUploads.length > 0" class="grid grid-cols-2 gap-4">
            <div v-for="image in previousUploads" :key="image.profile_image_id"
              ...
              <img :src="image.profile_image_url"
                @click="confirmSetAsProfileImage(image.profile_image_id, image.profile_image_url)" alt="Uploaded Image"
...
const setAsProfileImage = async (imageId, imageUrl) => {

  const currentProfileImageUrl = localStorage.getItem('user_profile_picture');

  const baseUrl = 'http://localhost:3000';
  const setProfileImage = baseUrl + currentProfileImageUrl;

  // Check if the image is already set as the profile picture
  if (imageUrl === setProfileImage) {
    toast.warn("This image is already your profile picture.");
    return;
  }
...
const confirmSetAsProfileImage = (imageId, imageUrl) => {
  const confirm = window.confirm("Are you sure you want to set this image as your profile picture?");
  if (confirm) {
    setAsProfileImage(imageId, imageUrl);  // Proceed to set the profile image if confirmed
    emit('image-uploaded');
    emit('close');
  }
};
