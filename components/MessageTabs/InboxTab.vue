<template>
    <div class="flex-1 px-2 py-3">
        ...
        <div class="flex-1 px-2 py-3 bg-gray-100">
            <ul>
                <span v-if="messages.length === 0 && !isLoading" class="text-center px-2">No Messages.</span>
                ...
                <li v-else v-for="(message, index) in messages" :key="message.message_id"
                    ...
                    <input @click.stop type="checkbox" v-model="selectedMessages" :value="message.message_id"
                        class="mr-2 focus:ring-0 border-2 border-gray-400" />
                    <div
                        class="w-full flex flex-col sm:flex-row items-start sm:items-center justify-between p-1 my-1 cursor-pointer">
                        ...
                        <div class="w-full sm:w-auto lg:w-auto">
                            <!-- Subject -->
                            <span class="sm:w-64 lg:w-48 w-full truncate">{{ message.subject }}</span>
                        </div>

                        <div class="w-full sm:w-auto ">
                            <!-- Message Content -->
                            <span class="w-full text-sm text-gray-600 truncate">{{ message.message }}</span>
                        </div>

                        <!-- For Larger Screens (Horizontal) -->
                        <div class="sm:w-32 flex items-center justify-end">
...
const isLoading = computed(() => inboxStore.isLoading);

const toggleCheckAll = () => {
    selectedMessages.value = checkAll.value ? messages.value.map(m => m.message_id) : [];
};
...
const markAsReadMessageUsingCheckbox = async () => {
    ...
    selectedMessages.value = []; // Clear selection after processing
};

const markAsUnread = async (messageId) => inboxStore.markAsUnread(messageId);

// For Marking as Unread on selected messages (checkbox)
const markAsUnreadMessageUsingCheckbox = async () => {
    if (selectedMessages.value.length === 0) {
        toast.error("Please select messages to mark as unread.");
        return;
    }
...
const spamMessageUsingCheckbox = async () => {
    if (selectedMessages.value.length === 0) {
        toast.error("Please select messages to report as spam.");
        return;
    }
...
const trashMessageUsingCheckbox = async () => {
    if (selectedMessages.value.length === 0) {
        toast.error("Please select messages to move to trash.");
        return;
    }
