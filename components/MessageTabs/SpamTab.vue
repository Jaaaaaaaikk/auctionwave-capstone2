<template>
    <div class="flex-1 px-2 py-3">
        <h1 class="text-center my-4">
            <strong>SPAM TAB</strong>
        </h1>
        <div class="bg-gray-100 mb-6">
            <ul>
                <span v-if="messages.length === 0 && !isLoading" class="text-center px-2">No
                    Spam Messages.</span>
...
const fetchUserSpam = async () => {
    isLoading.value = true;
    try {
        await new Promise(resolve => setTimeout(resolve, 1500));
        const response = await axios.get('/api/messages/fetch-message-user-spam');
